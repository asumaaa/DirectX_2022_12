...
SphereSprite* SphereSprite::GetInstance()
{
	static SphereSprite instance;
	return &instance;
}

void SphereSprite::Initialize(const wchar_t* szFile, DirectXCommon* dx, int texNum)
{
	this->dx_ = dx;
...
void SphereSprite::Initialize(DirectXCommon* dx, int texNum)
{
	this->dx_ = dx;
	HRESULT result;
	UINT incrementSize = dx_->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);	//デスクリプタのサイズ
...
void SphereSprite::SetImageData(XMFLOAT4 color)
{
	HRESULT result;
	for (size_t i = 0; i < imageDataCount; i++)
	{
