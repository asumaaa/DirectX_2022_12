...
SphereModel* SphereModel::GetInstance()
{
	static SphereModel instance;
	return &instance;
}

void SphereModel::Initialize(XMFLOAT3 size, DirectXCommon* dx_)
{
	dx = dx_;
	HRESULT result;
...
#pragma region 頂点初期化
void SphereModel::InitializeVertex(XMFLOAT3 size)
{
	angleY = 0;
	//頂点データ
	float x, y, z;
...
void SphereModel::InitializeIndexBuff()
{
	HRESULT result;
...
void SphereModel::InitializeVertBuff()
{
	HRESULT result;
...
#pragma endregion
#pragma region シェーダ
void SphereModel::CompileShader(const wchar_t* file, const wchar_t* file2)
{
...
void SphereModel::InitializeRootSignature()
{
	HRESULT result;
...
void SphereModel::InitializePipeline()
{
	HRESULT result;
...
void SphereModel::InitializePipelineState()
{
	HRESULT result;
	result = dx->GetDevice()->CreateGraphicsPipelineState(&pipelineDesc, IID_PPV_ARGS(&pipelineState));
	assert(SUCCEEDED(result));
